...
(() => {
    ...
    const render = () => {
        ...
        sections.forEach(section => {
            ...
            if (pager) {
                pager.hidden = false;
                pager.innerHTML = `<button type="button" data-page="-1" ${page === 1 ? 'disabled' : ''}>上一页</button><span>第 ${page}/${total} 页 · 共 ${items.length} 条</span><button type="button" data-page="1" ${page === total ? 'disabled' : ''}>下一页</button>`;
