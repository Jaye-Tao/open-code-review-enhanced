...
| `--no-filter` | — | `false` | Сохраняет все комментарии ревью и пропускает вызов LLM постобработки `REVIEW_FILTER_TASK` для каждой подзадачи. Подзадача проверяет один файл или набор связанных файлов. |
...
| `--default-prompt` | — | `false` | Использовать встроенный промпт ревью без получения пути к файлу. Можно сочетать с `--background`, но нельзя с `--background-file`. |
...
| `--max-tools <n>` | — | значение шаблона | Максимальное число раундов вызова инструментов для каждой подзадачи. `0` использует значение шаблона (`100`); значения 1–49 повышаются до `50`; итоговое значение применяется только если оно **больше** значения шаблона (предел можно лишь повысить, но не понизить). |
| `--max-tokens <n>` | — | значение конфигурации или шаблона | Предел токенов **промпта** для каждой подзадачи (по умолчанию `200000` для review). Переопределяет сохранённое значение `max_tokens` для этого запуска. На предел вывода не влияет — он задаётся отдельно параметром `MAX_COMPLETION_TOKENS` (`16384`). |
| `--max-tokens-budget <n>` | — | `0` (без ограничения) | Ограничивает общее число входных + выходных токенов ревью. Проверяется перед каждым раундом LLM: подзадача, уже превысившая бюджет, получает один финальный раунд, чтобы отправить найденное, и отмечается как `failed(budget)`; новые подзадачи не запускаются, а частичные результаты всё равно публикуются. |
...
Собирает одну сессию в единый автономный HTML-файл. Стили и скрипт
просмотрщика встраиваются в страницу, поэтому файл открывается по `file://`
без какого-либо сетевого доступа, а CI может сохранить ревью как артефакт сборки.
...
Без id сессии экспортируется самая свежая сессия репозитория. Такое
поведение выбрано по умолчанию, потому что успешный `ocr review` не печатает
id сессии. Без `-o` HTML выводится в stdout.

Экспортированная страница содержит фрагменты исходного кода, записанные
сессией, поэтому перед публикацией обращайтесь с ней так же аккуратно,
как с самим репозиторием.
